...
from lara_full.utils.video import get_all_frames, get_frames_by_timestamps
    ...
    def update_metadata(self, metadata_config: dict) -> None:
        ...
        for tag, metadatas in all_metadatas.items():
            self.merged_metadata[tag] = self.merge_metadata(
                metadatas=metadatas,
                dataset_sampling_weights=np.ones(len(metadatas)).astype(np.float32).tolist(), #self.dataset_sampling_weights.tolist(), ######ATTEN: assume all datasets have different embodiments, same weights for each one
