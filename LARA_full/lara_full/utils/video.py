...
def get_frames_by_timestamps(
    ...
    if video_backend == "decord":
        ...
        # num_frames = reader.get_metadata()["video"]["num_frames"]
        # fps = reader.get_metadata()["video"]["fps"]
        # frame_ts = np.arange(num_frames) / fps
        # desired_ts = frame_ts[indices]
        try:
            ...
            for frame in reader:
                ...
                #     break
            while len(loaded_frames) < 2:
                loaded_frames.append(frame["data"])
                loaded_ts.append(current_ts)
            assert len(loaded_frames) == 2
        except av.error.InvalidDataError:
            print(f"Invalid data while decoding {video_path}") ##chunk_001/1814 read error
        except Exception as e:
            print(f"Error during frame reading {video_path}: {e}")
