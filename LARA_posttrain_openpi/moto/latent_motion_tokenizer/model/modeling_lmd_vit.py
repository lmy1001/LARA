...
class LMDViTEmbeddings(nn.Module):
    def __init__(self, config: ViTConfig) -> None:
        ...
        self.query_pooling_layer = (
            nn.Linear(query_num * config.hidden_size, config.hidden_size) if query_fusion_mode == "add" else None
        )
...
class LMDViTModel(ViTPreTrainedModel):
    ...
    def forward(
        ...
        latent_motion_tokens: torch.Tensor | None = None,  # change lam_tokens to latent_motion_tokens
        ...
        embedding_output = self.embeddings(pixel_values=pixel_values, latent_motion_tokens=latent_motion_tokens)
