...
from lara_full.data.schema import DatasetMetadata
from lara_full.data.transform.base import InvertibleModalityTransform
...
class GR00TTransform(InvertibleModalityTransform):
    ...
    def apply_single(self, data: dict) -> dict:
        ...
        batch_data = {"images": images[:, :1], "language": language}
        vlm_outputs = self._apply_vlm_processing(batch_data)

        rgb_mean = np.array([0.485, 0.456, 0.406], dtype=np.float32)
        rgb_std = np.array([0.229, 0.224, 0.225], dtype=np.float32)
        _, _, channels, _, _ = images.shape
        rgb_mean = rgb_mean.reshape(1, 1, channels, 1, 1)
        rgb_std = rgb_std.reshape(1, 1, channels, 1, 1)
        ...
        transformed_data["state"] = state[:1]
        transformed_data["state_mask"] = state_mask[:1]
