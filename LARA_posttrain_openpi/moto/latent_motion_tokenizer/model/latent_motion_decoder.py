...
from moto.latent_motion_tokenizer.model.modeling_lmd_vit import LMDViTModel
...
class LatentMotionDecoder(nn.Module):
    ...
    def forward(
        self, cond_input: torch.Tensor | None = None, latent_motion_tokens: torch.Tensor | None = None
    ) -> torch.Tensor:
        outputs = self.transformer(
            pixel_values=cond_input, latent_motion_tokens=latent_motion_tokens, output_attentions=True
        )
        ...
        if not self.is_io_hidden_states:
            ...
            sequence_output = (
                sequence_output.permute(0, 2, 1).contiguous().reshape(batch_size, num_channels, height, width)
            )
