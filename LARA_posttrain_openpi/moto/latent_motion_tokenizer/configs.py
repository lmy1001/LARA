...
class LatentMotionTokenizerConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        image_encoder_config: DictConfig | None = None,
        m_former_config: DictConfig | None = None,
        vector_quantizer_config: DictConfig | None = None,
        decoder_config: DictConfig | None = None,
        hidden_state_decoder_config: DictConfig | None = None,
        **kwargs,
    ):
        ...
        self.image_encoder_config = (
            OmegaConf.to_container(image_encoder_config)
            if isinstance(image_encoder_config, DictConfig)
            else image_encoder_config
        )
        self.m_former_config = (
            OmegaConf.to_container(m_former_config) if isinstance(m_former_config, DictConfig) else m_former_config
        )
        self.vector_quantizer_config = (
            OmegaConf.to_container(vector_quantizer_config)
            if isinstance(vector_quantizer_config, DictConfig)
            else vector_quantizer_config
        )
        self.decoder_config = (
            OmegaConf.to_container(decoder_config) if isinstance(decoder_config, DictConfig) else decoder_config
        )
        self.hidden_state_decoder_config = (
            OmegaConf.to_container(hidden_state_decoder_config)
            if isinstance(hidden_state_decoder_config, DictConfig)
            else hidden_state_decoder_config
        )
